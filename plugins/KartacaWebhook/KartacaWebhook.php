...
class KartacaWebhook extends PluginBase
{
    protected $storage = 'DbStorage';
    static protected $name = 'Kartaca - WebHook';
    static protected $description = 'Webhook plugin developed by Kartaca';
    ...
    public function getPluginSettings($getValues = true)
    {
        return [
            'webhookUrl' => [
                'type' => 'string',
                'label' => 'Webhook URL',
                'help' => 'The external system address to which data will be sent. E.g., https://example.com/hook',
                'default' => '',
                'current' => $this->get('webhookUrl'),
            ],
            'authHeaderKey' => [
                'type' => 'string',
                'label' => 'Auth Header Key',
                'help' => 'The auth header key required for the webhook URL. E.g., X-Auth-Key',
                'default' => '',
                'current' => $this->get('authHeaderKey'),
            ],
            'authHeaderValue' => [
                'type' => 'string',
                'label' => 'Auth Header Value',
                'help' => 'The auth header value required for the webhook URL. E.g., 1234567890',
                'default' => '',
                'current' => $this->get('authHeaderValue'),
            ],
            ...
            'enableEventLogging' => [
                ...
                'help' => 'Determines whether event logs are saved or not',
                'default' => true,
            ...
            'enableJsonLogging' => [
                ...
                'help' => 'Determines whether JSON data logs are saved or not',
                'default' => true,
    ...
    private function logEvent(string $text): void
    {
        if ($this->get('enableEventLogging', null, null, true)) {
            ...
            @file_put_contents($this->getLogPath(), "[$timestamp] $text\n", FILE_APPEND | LOCK_EX);
        }
    }
    ...
    private function sendWebhook(array $payload): void
    {
        ...
        if (!$webhookUrl) {
            $this->logEvent('Webhook URL not configured, skipping: ' . ($payload['event'] ?? 'unknown'));
            return;
        }
        $headers = ['Content-Type: application/json'];
        $authKey = $this->get('authHeaderKey', null, null, '');
        $authValue = $this->get('authHeaderValue', null, null, '');
        if (!empty($authKey) && !empty($authValue)) {
        ...
        curl_close($ch);
        if ($error) {
            $this->logEvent('Webhook FAILED: ' . ($payload['event'] ?? 'unknown') . ' - cURL Error: ' . $error);
        } elseif ($httpCode >= 200 && $httpCode < 300) {
            $this->logEvent('Webhook SUCCESS: ' . ($payload['event'] ?? 'unknown') . ' - HTTP ' . $httpCode);
        } else {
            $this->logEvent('Webhook FAILED: ' . ($payload['event'] ?? 'unknown') . " - HTTP $httpCode - Response: $response");
        }
    }
    ...
    private function resolveSubquestionText($subq, $fallback)
    {
        if ($subq && isset($subq->question)) {
            return strip_tags($subq->question);
    ...
    public function beforeSurveyPage()
    {
        ...
        $startedFlagKey   = 'kartaca_started_sent_' . $surveyId;
    ...
    public function beforeControllerAction()
    {
        ...
        if ($controller === 'survey' && $action === 'index' && $_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (!empty($answers)) {
                ...
                $this->sendWebhook($submitPayload);
    ...
    private function extractAnswersFromPost($surveyId, $step, $postData): array
    {
        ...
        foreach ($postData as $key => $value) {
            if (!preg_match('/^(\d+)X(\d+)X(\d+)(SQ\d+)?$/', $key, $matches)) {
                continue;
            }
            $qid = (int)$matches[3];
            $subCode = $matches[4] ?? null;
    ...
    private function extractAnswers($surveyId, $responseModel): array
    {
        ...
        try {
            ...
            foreach ($attributes as $fieldName => $value) {
                ...
                if (!preg_match('/^(\d+)X(\d+)X(\d+)(SQ\d+)?$/', $fieldName, $matches)) {
                    continue;
                }
                $qid = (int)$matches[3];
                $subCode = $matches[4] ?? null;
    ...
    private function killAbandonedSessionSafe(string $storedSessionId): void
    {
        ...
        try {
            ...
            if ($handler === 'files' && $savePath) {
                ...
                if (is_file($file)) {
                    @unlink($file);
                    $this->logEvent("[SELFSCAN] Killed PHP session file: $file");
                }
            }
        } catch (Throwable $e) {
    ...
    private function killPhpSessionById($phpSid): void
    {
        ...
        try {
            ...
            if ($handler === 'files' && $savePath) {
                ...
                if (is_file($file)) {
                    @unlink($file);
                    $this->logEvent("[SELFSCAN] Killed PHP session file by php_sid: $file");
                }
            }
        } catch (Throwable $e) {
