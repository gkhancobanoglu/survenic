...
document.addEventListener('DOMContentLoaded', function () {
    document.querySelectorAll('.table-array-radio, .table-10-point-array').forEach(function(tbl){
      var ths = Array.from(tbl.querySelectorAll('thead th')).slice(1).map(function(th){
        return th.textContent.trim();
      });
      tbl.querySelectorAll('tbody tr').forEach(function(tr){
        Array.from(tr.querySelectorAll('td')).forEach(function(td, i){
          if (i === 0) return;
          var lab = td.querySelector('label');
          if (lab) lab.setAttribute('data-col', ths[i-1] || '');
        });
      });
    });
  });
