...
<div id='publication-panel'>
    ...
    <div>
        <!-- Link survey on public index page (ADMIN ONLY) -->
        <?php if (Permission::model()->hasGlobalPermission('superadmin','read')): ?>
        <div class="mb-3 mt-4">
            <h1><?php eT("Access control"); ?></h1>
            <label class="form-label" for='listpublic'>
                <?php printf(gT("Link survey on %spublic index page%s:"), "<a href='" . Yii::app()->getConfig("publicurl") . "' target='_blank'>", "</a>"); ?>
            </label>
            <div>
        ...
        <!-- Set cookie to prevent repeated participation (VISIBLE TO ALL) -->
        <div class="mb-3">
        ...
        <!-- CAPTCHA Settings -->
        <?php
        $usecap = $oSurvey->usecaptcha;
        $aCaptchaSurveyAccessYes     = ['A', 'B', 'C', 'X', 'F', 'H', 'K', 'O', 'T'];
        $aCaptchaSurveyAccessInherit = ['E', 'G', 'I', 'J', 'L', 'M', '1', '2', '4'];
        $aCaptchaRegistrationYes     = ['A', 'B', 'D', 'R', 'F', 'G', 'I', 'M', 'U'];
        $aCaptchaRegistrationInherit = ['E', 'H', 'J', 'K', 'O', 'P', '1', '3', '6'];
        $aCaptchaLoadSaveYes         = ['A', 'C', 'D', 'S', 'G', 'H', 'J', 'L', 'P'];
        $aCaptchaLoadSaveInherit     = ['E', 'F', 'I', 'K', 'T', 'U', '2', '3', '5'];
        ?>

        <div class="mb-3 mt-4">
            <h1><?php eT("CAPTCHA"); ?></h1>

            <!-- CAPTCHA: Survey access -->
            ...
            <div>
                <?php $this->widget('ext.ButtonGroupWidget.ButtonGroupWidget', [
                    ...
                    'checkedOption' => (in_array($usecap, $aCaptchaSurveyAccessYes)) ? 'Y' : ((in_array($usecap, $aCaptchaSurveyAccessInherit)) ? 'I' : 'N'),
                    'selectOptions' => ($bShowInherited)
            ...
            <label class="form-label mt-3" for='usecaptcha_registration'><?php eT("Use CAPTCHA for registration:"); ?></label>
            <div>
                <?php $this->widget('ext.ButtonGroupWidget.ButtonGroupWidget', [
                    'name'          => 'usecaptcha_registration',
                    'checkedOption' => (in_array($usecap, $aCaptchaRegistrationYes)) ? 'Y' : ((in_array($usecap, $aCaptchaRegistrationInherit)) ? 'I' : 'N'),
                    'selectOptions' => ($bShowInherited)
            ...
            <!-- CAPTCHA: Save and load (ADMIN ONLY) -->
            <?php if (Permission::model()->hasGlobalPermission('superadmin','read')): ?>
            <label class="form-label mt-3" for='usecaptcha_saveandload'><?php eT("Use CAPTCHA for save and load:"); ?></label>
            <div>
                <?php $this->widget('ext.ButtonGroupWidget.ButtonGroupWidget', [
                    'name'          => 'usecaptcha_saveandload',
                    'checkedOption' => (in_array($usecap, $aCaptchaLoadSaveYes)) ? 'Y' : ((in_array($usecap, $aCaptchaLoadSaveInherit)) ? 'I' : 'N'),
                    'selectOptions' => ($bShowInherited)
